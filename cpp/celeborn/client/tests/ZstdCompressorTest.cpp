...
TEST(ZstdCompressorTest, CompressWithZstd) {
  for (int compressionLevel = -5; compressionLevel <= 22; compressionLevel++) {
    compress::ZstdCompressor compressor(-5);
    ...
    const auto decompressedData = new uint8_t[oriLength + 1];
    decompressedData[oriLength] = '\0';
    const bool success =
        decompressor.decompress(compressedData.data(), decompressedData, 0);
    EXPECT_TRUE(success);
    EXPECT_EQ(reinterpret_cast<char*>(decompressedData), toCompressData);
